...
const values =
  (tableName: TableName, columns: ColumnName[]) => (values: string[][]) => {
    if (
      values.map((v) => v.length).filter((v) => v != columns.length).length > 0
    ) {
...
const value =
  (tableName: TableName, columns: ColumnName[]) => (value: string[]) => {
...
const table = () => (table: TableName) => ({ columns: columns(table) });
